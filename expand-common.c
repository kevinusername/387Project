...
static bool set_extend_size(uintmax_t tabval) {
    bool ok = true;
...
static bool set_increment_size(uintmax_t tabval) {
    bool ok = true;
...
extern void finalize_tab_stops(void) {
    ...
    if (first_free_tab == 0)
        tab_size = max_column_width =
            extend_size ? extend_size : increment_size ? increment_size : 4;
...
extern uintmax_t get_next_tab_column(const uintmax_t column, size_t *tab_index,
    ...
    *last_tab = false;
    ...
    if (increment_size) {
        ...
        return column +
               (increment_size - ((column - end_tab) % increment_size));
    }
